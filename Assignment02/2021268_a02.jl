...
function bisection(f, a, b; iters=1000, TOL=1e-3)
	...
	if sign(f(a₁)) == sign(f(b₁))
		error("Does not satisfy the IVT theorem!")
	end
...
let
	root = bisection(x -> x^3 + 4x^2 - 10, 1, 2, TOL=1e-4)
	md"Root of $f(x) = x^3 + 4x^2 - 10$ is $root:
	
	"
end

# ╔═╡ 5aa6f053-56a8-47ab-a104-582b9771aec9
function fixed_point(g, x₀; iters=1000, TOL=1e-6)
	x = x₀
	for _=1:iters
		xᵢ = g(x)
		println(xᵢ)
...
let
	root = fixed_point(x -> 1 / (x + 1), 0)
	md"Root of $f(x) = \frac{1}{x + 1}$ is $root
	
	"
end

# ╔═╡ 28bd88f9-be4c-41a7-ac94-ea2b26743256
function secant(f, x₁, x₂; iters=1000, TOL=1e-6)
	
	for _=1:iters
		numerator = f(x₂)
		denominator = (f(x₂) - f(x₁)) / (x₂ - x₁)

		xₙ = x₂ - numerator/denominator
		if abs(xₙ - x₂) < TOL
			return xₙ
		end
		x₁ = x₂
...
# ╔═╡ 3c52b846-86ca-4f56-9395-9cbc336c217c
secant(x -> x^7 - 1000, 2, 3, iters=100)

# ╔═╡ 74ba6c30-30ad-4607-b0a6-2fb0fb15a142
function next_secant(f, x₁, x₂)
	numerator = f(x₂)
	denominator = (f(x₂) - f(x₁)) / (x₂ - x₁)
...
begin
	...
	to_radians(θ) = θ * (π / 180)
	
	x_vals = -10:0.01:10
	y_vals = f.(x_vals)
	
	plot!(p, x_vals, y_vals, label="sin(cos(eˣ))", linewidth=2, legend=false)
	hline!([0], color=:black, linewidth=1, label="y=0")
